...
Extracts 768-dimensional general-purpose audio embeddings
"""
...
class Wav2Vec2EmbeddingExtractor:
    def __init__(self, model_name="facebook/wav2vec2-base", device='cuda' if torch.cuda.is_available() else 'cpu'):
        """
        Initialize Wav2Vec2 embedding extractor
        ...
        print(f"Loading Wav2Vec2 model: {model_name} on {device}...")
    ...
    def extract_embedding(self, audio_path, segment_length=3.0, hop_length=1.5):
        ...
        # Resample to 16kHz if necessary (Wav2Vec2 expects 16kHz)
    ...
    def process_single_file(self, audio_path, output_dir):
        ...
        if result:
            ...
            output_file = output_path / f"{audio_file.stem}_wav2vec2_embeddings.json"
    ...
    def process_directory(self, input_dir, output_dir):
        ...
        # Save summary
        summary_file = output_path / "wav2vec2_summary.json"
        summary = {
            'model': 'Wav2Vec2',
            'model_name': self.model_name,
            'embedding_dim': 768,
...
def main():
    ...
    extractor = Wav2Vec2EmbeddingExtractor(model_name=args.model_name, device=device)
