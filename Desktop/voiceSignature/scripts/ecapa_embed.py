#!/usr/bin/env python3
"""
SpeechBrain ECAPA-TDNN Embedding Extractor
Extracts 192-dimensional speaker verification embeddings
"""
...
class ECAPAEmbeddingExtractor:
    def __init__(self, device='cuda' if torch.cuda.is_available() else 'cpu'):
        """
        Initialize ECAPA-TDNN embedding extractor
        ...
        # Load the ECAPA-TDNN model for speaker verification
    ...
    def extract_embedding(self, audio_path, segment_length=3.0, hop_length=1.5):
        ...
        for start in range(0, len(signal) - segment_samples, hop_samples):
            ...
            with torch.no_grad():
                embedding = self.model.encode_batch(segment.unsqueeze(0))
        ...
        with torch.no_grad():
            full_embedding = self.model.encode_batch(signal.unsqueeze(0))
            full_embedding = full_embedding.squeeze().cpu().numpy()
    ...
    def process_single_file(self, audio_path, output_dir):
        ...
        if result:
            ...
            output_file = output_path / f"{audio_file.stem}_ecapa_embeddings.json"
    ...
    def process_directory(self, input_dir, output_dir):
        ...
        # Save summary
        summary_file = output_path / "ecapa_summary.json"
        summary = {
            'model': 'ECAPA-TDNN',
            'embedding_dim': 192,
...
def main():
    parser = argparse.ArgumentParser(description='Extract ECAPA-TDNN embeddings from audio files')
    parser.add_argument('--input_dir', type=str, default='../audio', 
                       help='Directory containing audio files')
    parser.add_argument('--input_file', type=str,
                       help='Single audio file to process (overrides input_dir)')
    parser.add_argument('--output_dir', type=str, default='../outputs/ecapa',
    ...
    extractor = ECAPAEmbeddingExtractor(device=device)
